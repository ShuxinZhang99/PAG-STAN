...
from model.My_Model.GraphLSTM_cell import ConvLSTMCell


class GConvLSTM(nn.Module):

    def __init__(self, input_size, input_dim, hidden_dim, num_nodes, num_layers, batch_first=True):
        """
    ...
    def forward(self, input_x, adj, state=None):
        """
        ...
        if not self.batch_first:
            # (t, b, h, w) -> (batch_size, time_step, channels, height, weight)
        ...
        for layer in range(self.num_layers):
            ...
            for t in range(time_step):
                cur_input = input_x[:, t, :, :, :]
                c, h = self.cell_list[layer](xt=cur_input, adj=adj, state=(c, h))
                t_output.append(h)
