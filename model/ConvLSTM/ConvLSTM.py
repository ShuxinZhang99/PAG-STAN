...
from model.ConvLSTM.ConvLSTM_cell import ConvLSTMCell


class ConvLSTM(nn.Module):

    def __init__(self, input_size, input_dim, hidden_dim, kernel_size,
                 num_layers, padding="SAME", stride=1, dilation=1, bias=True, batch_first=True):
        """
    ...
    def forward(self, input_x, state=None):
        ...
        for layer in range(self.num_layers):
            ...
            for t in range(time_step):
                cur_input = input_x[:, t, :, :, :]  # (batch, 1, station, station)
