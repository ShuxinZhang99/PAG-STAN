...
path = 'D:/ZSX/Paper3/save_model/CSTN-60min_2023_01_27_20_19_46/model_dict_checkpoint_103_0.00001466.pth'
...
with torch.no_grad():
    model.eval()
    test_loss = 0
    # for OD_te, Flow_te, ODRate_te in zip(enumerate(OD_data_loader_test), enumerate(inflow_data_loader_test), enumerate(ODRatio_data_loader_test)):
    for OD_te in enumerate(OD_data_loader_test):
        i_batch, (test_OD_X, test_OD_Y, test_OD_Y_original) = OD_te
        #i_batch, (test_Flow_X, test_Flow_Y, test_Flow_Y_original) = Flow_te
        #i_batch, (test_ODRate_X, test_ODRate_Y, test_ODRate_Y_original) = ODRate_te
        test_OD_X, test_OD_Y = test_OD_X.type(torch.float32).to(device), test_OD_Y.type(torch.float32).to(device)
        #test_Flow_X, test_Flow_Y = test_Flow_X.type(torch.float32).to(device), test_Flow_Y.type(torch.float32).to(device)
        #test_ODRate_X, test_ODRate_Y = test_ODRate_X.type(torch.float32).to(device), test_ODRate_Y.type(torch.float32).to(device)

        # target = model(test_inflow_X, test_outflow_X, adjacency)
        target = model(test_OD_X) #, test_Flow_X, test_ODRate_X, adjacency)  # , test_weibo_X, test_confirmed_X)  # Graph: edge_index)
        # target = torch.masked_fill(input=target, mask=~mask, value=0).to(device)
        # test_inflow_Y = torch.masked_fill(input=test_inflow_Y, mask=~mask, value=0).to(device)
        loss = mse(input=test_OD_Y, target=target)
        test_loss += loss.item()

        # evaluate on original scale
        # 获取result (batch, 276, pre_len)
        clone_prediction = (target.cpu().detach().numpy().copy() * (max_OD - min_OD)) + min_OD  # clone(): Copy the tensor and allocate the new memory
...
x_11.to_csv('result/result_cnn/OD_Prediction.csv', encoding='gbk')
y_11 = pd.DataFrame(y, columns=None)
y_11.to_csv('result/result_cnn/OD_Actual.csv', encoding='gbk')
...
sns.heatmap(x, ax=ax, vmin=0, vmax=30, cmap='YlOrRd', annot=False, linewidths=2, cbar=True)
